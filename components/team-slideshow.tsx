...
export default function TeamSlideshow() {
  ...
  function handleImageError(e: React.SyntheticEvent<HTMLImageElement, Event>, title: string) {
    e.currentTarget.src = `/placeholder.svg?height=500&width=800&text=${encodeURIComponent(title)}`
  }
  function handleThumbError(e: React.SyntheticEvent<HTMLImageElement, Event>, title: string) {
    e.currentTarget.src = `/placeholder.svg?height=80&width=120&text=${encodeURIComponent(title.slice(0, 10))}`
  }
            ...
            {teamImages.map((slide, index) => (
              ...
              >
                <Image
                  ...
                  onError={(e) => handleThumbError(e, slide.title)}
                />
